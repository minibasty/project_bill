...
<body>
	...
	$cancel_date=isset($check_row['cancel_date']) ? $check_row['cancel_date'] : '';
	if ($cancel_date == "") {
		$cancel_date="";
	}else{
		$cancel_date=DateDMY($cancel_date);
	}
	$cancel_reason=isset($check_row['reason_cancel']) ? $check_row['reason_cancel'] : '';
	if (isset($_POST['save'])) {
		$reason = $_POST['reason'];
		...
		$check_qr_update=$conn->query($check_cancel);		
		$check_row_update=$check_qr_update->fetch_array();
		echo $sql_update_member="UPDATE `member` SET `cancel` = '$check_row_update[id_cancel]' WHERE id = '$check_row_update[member_id]'";
		$update_concel_qr=$conn->query($sql_update_member);

		if ($update_concel_qr) {
			// echo "OK Insert";
			if ($update_concel_qr) {
				print "<meta http-equiv=refresh content=0;URL=?p=canceler&user=".$_GET['user'].">";
			}
		}
	}
	?>
