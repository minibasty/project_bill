<head>
    <meta charset="utf-8">
    <title>:: Download Report Setup</title>
...
$spreadsheet->getActiveSheet()->getColumnDimension('M')->setWidth(30.13);

// เพิ่มข้อมูล
$sheet = $spreadsheet->getActiveSheet();
$sheet->setCellValue('A1', 'รายงานประจำเดือน '.$monthStr.' '.$yearStr)->mergeCells('A1:M1')->getStyle('A1:M1')->applyFromArray($headStyle);
$sheet->setCellValue('A2', 'แบบรายงานการติดตั้ง เครื่องบันทึกข้อมูลการเดินทางของรถ')->mergeCells('A2:M2')->getStyle('A2:M2')->applyFromArray($headStyle);
$sheet->setCellValue('A3', 'โดย บริษัท .มิรดา คอร์ปอเรชั่น จำกัด... Vendor…84….')->mergeCells('A3:M3')->getStyle('A3:M3')->applyFromArray($headStyle);
...
$sheet->setCellValue('M5', 'หมายเหตุ')->mergeCells('M5:M6')->getStyle('M5')->applyFromArray($headMenu);
$sheet->getStyle('A5:M6')->applyFromArray($borderAll);
...
$endMonth=YearTh2($endMonth);
    ...
    {   
        ...
        if($_POST["chk"][$ii] !="")
        {
                ...
                $imei=$objResult['zipcode'];
                $chassi=$objResult['user'];
                ...
                $spreadsheet->getActiveSheet()->setCellValue('E' . $i, $chassi)->getStyle('E' . $i)->applyFromArray($content);
                ...
                $spreadsheet->getActiveSheet()->setCellValue('I' . $i, $imei)->getStyle('I' . $i)->applyFromArray($content);
                ...
                $spreadsheet->getActiveSheet()->getStyle('A' . $i.':M'.$i)->applyFromArray($borderAll);
                $spreadsheet->getActiveSheet()->getStyle('I' . $i)->getNumberFormat()->setFormatCode('####################');
                $spreadsheet->getActiveSheet()->getStyle('E' . $i)->getNumberFormat()->setFormatCode('####################');
        $i++;
        $num++;
        }
    }
...
// non-approve
$writer->save('excel/setup_report.xlsx');
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header bg-success text-white">
                    <div class="form-row">
                     <div class="col">
                        ดาวน์โหลดรายงานการติดตั้ง
                    </div>
                    <div class="col text-right">
                        <a href="?p=report" title=""><button type="button" class="btn btn-sm btn-warning">กลับหน้ารายงาน</button></a>
                    </div>
                    </div>
                </div>
                <div class="card-body text-center">
                    <a href="report/excel/setup_report.xlsx" title=""> <button type="button" class="btn btn-info">
